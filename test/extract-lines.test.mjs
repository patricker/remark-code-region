...
const sayHi = fs.readFileSync(
  path.join(__dirname, 'fixtures/snippets/say-hi.js'),
  'utf-8',
);
...
describe('extractLines — errors', () => {
  it('throws on L0 (below 1-based minimum)', () => {
    expect(() => extractLines(sayHi, 'L0', 'say-hi.js')).toThrow(
      'line 0 is out of range',
    );
  });

  it('throws on start > total lines', () => {
    expect(() => extractLines(sayHi, 'L99', 'say-hi.js')).toThrow(
      'line 99 is out of range',
    );
  });

  it('throws on end > total lines', () => {
    expect(() => extractLines(sayHi, 'L1-L99', 'say-hi.js')).toThrow(
      'line 99 is out of range',
    );
  });

  it('throws on reversed range (L6-L3)', () => {
    expect(() => extractLines(sayHi, 'L6-L3', 'say-hi.js')).toThrow(
      'start line 6 is after end line 3',
    );
  });

  it('throws on empty file (L2)', () => {
    // ''.split('\n') gives [''] — 1 element, so L1 is valid but L2 is out of range
    expect(() => extractLines('', 'L2', 'empty.js')).toThrow(
